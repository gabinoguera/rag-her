...
8- IMPORTANT: ALL file edits MUST use absolute paths within the worktree (`.trees/feature-local-$ARGUMENTS/`). NEVER edit files in the main working tree.
9- Generate implementation report in `.trees/feature-local-$ARGUMENTS/task_report.md` with:
...
10- At the end after the confirmation of the user, commit the changes locally (no push)
11- Remove the sentinel: `rm -f .claude/.worktrees_active/feature-local-$ARGUMENTS`
