...
At the end of this message, I will ask you to do something. Please follow the "Worktree Setup, Explore, Team Selection, Plan, Advice, Update, Clarification and Iterate" workflow when you start over the user_request.

# Worktree Setup
1. Generate a short, descriptive feature name from the user request (e.g., "genre-validation", "pdf-processing")
2. Create a new git worktree: `git worktree add ./.trees/{feature_name} -b {feature_name}`
3. Change to the worktree directory: `cd .trees/{feature_name}`

# Create the session file
Create `.claude/sessions/context_session_{feature_name}.md` inside the worktree (`.trees/{feature_name}/.claude/sessions/context_session_{feature_name}.md`) where plan is going to be updated with all the future iterations and feedback
